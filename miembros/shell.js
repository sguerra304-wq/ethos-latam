...
(function () {
  const ICONS = {
    ...
    herramientas: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z" stroke-linecap="round" stroke-linejoin="round"/></svg>',
  ...
  const NAV = [
    ...
    { key: "herramientas", href: "herramientas.html", label: "Herramientas" },
    { key: "beneficios", href: "beneficios.html", label: "Beneficios" },
    { key: "perfil", href: "perfil.html", label: "Perfil" }
  ];
  const BRAND = '<a href="index.html" class="brand"><img class="mark" src="/assets/img/logo.png" alt="" aria-hidden="true" width="34" height="34" decoding="async">Ethos <b>LATAM</b></a>';
  ...
  window.EthShell = {
    async mount(active) {
      ...
      try {
        if (window.EthData) {
          const cached = window.EthData.peek();
          if (cached && Object.keys(cached).length) {
            // Caché lista → render inmediato + revalidación en segundo plano (sin bloquear).
            snap = cached;
            window.EthData.refresh().catch(() => {});
          } else {
            // Primera carga de la sesión → sí esperamos el snapshot.
            snap = await window.EthData.refresh();
          }
        }
      } catch (e) {}
      ...
      document.getElementById("ethLogout").onclick = () => {
        try { window.EthData && window.EthData.clearCache(); } catch (e) {}
        window.EthAuth.signOut();
      };
      const setNav = (o) => {
