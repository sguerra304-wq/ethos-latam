...
export function defaultDB() {
  return {
    ...
    events: [
      { id: "ev-rio", date: "2026-11-15", title: "Ethos Río — Encuentro de lanzamiento", type: "Encuentro", city: "Río de Janeiro", mode: "Presencial", plan: "Starter", img: "../assets/img/hero-founders.webp", desc: "El primer encuentro de Ethos LATAM: 15 founders en Río de Janeiro para arrancar la comunidad. Íntimo y por aplicación.", seats: 15, reserved: 5 },
      { id: "ev-cusco", date: "2026-09-01", title: "Ethos Cusco — Residencia de 2 semanas", type: "Residencia", city: "Cusco", mode: "Presencial", plan: "Pro", img: "../assets/img/emotion-sunrise.webp", desc: "Dos semanas en Cusco para founders: convivir, trabajar y recargar en comunidad.", seats: 20, reserved: 0 },
      { id: "ev-ba", date: "2027-03-01", title: "Ethos Buenos Aires", type: "Encuentro", city: "Buenos Aires", mode: "Presencial", plan: "Starter", img: "../assets/img/emotion-celebration.webp", desc: "La comunidad Ethos se reúne en Buenos Aires. Marzo 2027.", seats: 80, reserved: 0 }
    ],
    resources: [
...
export async function readDB() {
  ...
  // Si la DB EXISTE pero no se pudo leer, hay que fallar: devolver defaultDB()
  // aquí haría que una escritura posterior pisara los datos reales con el seed.
  if (!res.ok) throw new Error("No se pudo leer la base de datos (HTTP " + res.status + "). Intenta de nuevo.");
  return res.json();
}
...
export function readJson(req) {
  // Tope de 1.5MB (el avatar base64 legítimo pesa ≤800KB); un body gigante se corta.
  const MAX = 1.5 * 1024 * 1024;
  return new Promise((resolve) => {
    let d = "";
    req.on("data", (c) => { d += c; if (d.length > MAX) { d = ""; try { req.destroy(); } catch (e) {} resolve({}); } });
    req.on("end", () => { try { resolve(d ? JSON.parse(d) : {}); } catch (e) { resolve({}); } });
  });
}

/* ---------- Rate limiting (en memoria, por instancia) ----------
   En serverless cada instancia tiene su propio mapa, así que es "best effort":
   no sustituye a un WAF, pero corta en seco fuerza bruta y spam de formularios
   desde una misma IP. Devuelve false cuando la clave superó el máximo. */
const _hits = new Map();
export function rateLimit(key, max, windowMs) {
  const now = Date.now();
  if (_hits.size > 5000) for (const [k, v] of _hits) { if (now > v.reset) _hits.delete(k); }
  const rec = _hits.get(key);
  if (!rec || now > rec.reset) { _hits.set(key, { n: 1, reset: now + windowMs }); return true; }
  rec.n++;
  return rec.n <= max;
}
export const clientIp = (req) =>
  (String(req.headers["x-forwarded-for"] || "").split(",")[0].trim() || "ip-desconocida");
